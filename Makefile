...
aptbuild:
	...
	sudo apt-get install --force-yes -y build-essential checkinstall devscripts dh-make fakeroot libxml-parser-perl check avahi-daemon vlc mplayer libxine1-ffmpeg gxine mencoder mpeg2dec vorbis-tools id3v2 mpg321 mpg123 libflac++6 ffmpeg totem-mozilla icedax tagtool easytag id3tool lame nautilus-script-audio-convert libmad0 libjpeg-progs flac faac faad sox ffmpeg2theora libmpeg2-4 uudeview flac libmpeg3-1 mpeg3-utils mpegdemux liba52-0.7.4-dev gstreamer0.10-ffmpeg gstreamer0.10-fluendo-mp3 gstreamer0.10-gnonlin gstreamer0.10-sdl gstreamer0.10-plugins-bad-multiverse gstreamer0.10-plugins-bad gstreamer0.10-plugins-ugly totem-plugins-extra gstreamer-dbus-media-service gstreamer-tools ubuntu-restricted-extras wine playonlinux unace rar unrar zip unzip p7zip-full p7zip-rar sharutils uudeview mpack lha arj cabextract file-roller multiget  empathy telepathy-mission-control-5 telepathy-gabble telepathy-haze telepathy-idle telepathy-salut telepathy-sofiasip msn-pecan skype banshee banshee-extension-appindicator banshee-extension-lyrics banshee-extension-mirage gimp gimp-data gimp-plugin-registry gimp-data-extras gedit gedit-plugins gedit-developer-plugins libreoffice libreoffice-pdfimport libreoffice-math libreoffice-wiki-publisher libreoffice-gnome deluge-torrent amule gsfonts gsfonts-x11 flashplugin-installer ssh subversion subversion-tools pptp-linux ppp pptpd network-manager-pptp denyhosts nmap psad chkrootkit logwatch
	sudo sed -i 's:(tmpfs).*::g' /etc/fstab
	sudo sh -c "echo '\n#Secure shared memory \ntmpfs     /dev/shm     tmpfs     defaults,noexec,nosuid     0     0\n' >> /etc/fstab"
	sudo sed -i 's_(PermitRootLogin).*_PermitRootLogin no_g' /etc/ssh/sshd_config
	sudo usermod -a -G sudo salimane && sudo dpkg-statoverride --update --add root sudo 4750 /bin/su
...
devbuild:
	...
	sudo apt-get update; \
	sudo apt-get install --force-yes -y mysql-server mysql-client libmysqlclient-dev graphicsmagick-libmagick-dev-compat libc6 libpcre3 libpcre3-dev libpcrecpp0 libssl0.9.8 libssl-dev zlib1g zlib1g-dev lsb-base nginx php5-common php5-dev php5-cli php5-mysql php5-curl php5-gd php5-intl php-pear php5-imagick php5-imap php5-mcrypt php5-memcache php5-ming php5-pspell php5-recode php5-snmp php5-sqlite php5-tidy php5-xmlrpc php5-xsl php5-xdebug php5-geoip php5-memcached php5-fpm php5-cgi python python-setuptools python-dev python-software-properties python-pip redis-server libpcre3-dev heroku-toolbelt graphviz ruby1.9.1 ruby1.9.1-dev libreadline-dev libruby1.9.1 sqlite3-pcre libsqlite3-dev sqlite3 libsqlite3-ruby1.9.1  libbz2-dev zlib1g-dev tokyotyrant tokyotyrant-utils libxml2-dev psmisc libxml2 
	mysql_secure_installation
	if test ! -d ~/htdocs/redis; then cd ~/htdocs && git clone https://github.com/antirez/redis.git; fi
	if test ! -f /usr/bin/redis-server; then cd ~/htdocs/redis && git checkout 2.6 && \
	make && \
	make test && \
